...
class SplashView : View {
    ...
    private var mDuration =
        DEFAULT_DURATION // total duration, in ms, of the animation
            .toLong()
    ...
    private fun setupAttributes(attrs: AttributeSet?) {
        val context = context
        val a = context.obtainStyledAttributes(attrs, R.styleable.TwitterSplashView)
        val numAttrs = a.indexCount
        for (i in 0 until numAttrs) {
            val attr = a.getIndex(i)
            when (attr) {
                R.styleable.TwitterSplashView_icon -> setIconDrawable(a.getDrawable(i))
                R.styleable.TwitterSplashView_iconColor -> setIconColor(
                    a.getColor(
                        i,
                        DEFAULT_ICON_COLOR
                    )
                )
                R.styleable.TwitterSplashView_holeFillColor -> setHoleFillColor(
                    a.getColor(
                        i,
                        DEFAULT_HOLE_FILL_COLOR
                    )
                )
                R.styleable.TwitterSplashView_duration -> setDuration(
                    a.getInt(
                        i,
                        DEFAULT_DURATION
                    ).toLong()
                )
                R.styleable.TwitterSplashView_removeFromParentOnEnd -> setRemoveFromParentOnEnd(
                    a.getBoolean(
                        i,
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val iconHeight = mIconHeight * mCurrentScale
    ...
    companion object {
        ...
        const val DEFAULT_HOLE_FILL_COLOR = Color.WHITE
        val DEFAULT_ICON_COLOR = Color.rgb(23, 169, 229)
        const val DEFAULT_DURATION = 500
        const val DEFAULT_REMOVE_FROM_PARENT_ON_END = true
        private const val PAINT_STROKE_WIDTH =
